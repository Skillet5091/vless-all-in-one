...
5. 每次修改并提交时，必须同步更新版本号。
6. 版本号规则：
   - 修复 / 小改动 / 文档调整 / 兼容性修复 → **小版本号递增**（如 `3.4.10` → `3.4.11`）
   - 新功能 / 功能增强 / 明显功能更新 → **中版本号递增**（如 `3.4.10` → `3.5.0`）
   - 重构 / 大改版 / 不兼容变更 / 架构调整 → **大版本号递增**（如 `3.4.10` → `4.0.0`）
7. 如果一次提交同时包含多类变更，按影响最大的级别提升版本号。
