...
- 🛡️ **WARP 出口稳定性修复**
  - 移除每 15 分钟强制断开/重连造成的出口 IP 轮换
  - WARP 健康检查仅在连续失败后重连，并增加冷却时间
  - 保持 Twitter/X 等长连接，避免切换出口造成卡顿

### v3.7.0 (2026-07-15)

- 🌐 **xAI/Grok Cloudflare 出口分流**
  - 新增 `x.ai` 和 `grok.com` 预设域名规则
  - 可在分流管理中将 xAI/Grok 流量单独指向 WARP（Cloudflare IP）
  - Xray 与 sing-box 均会生成对应的域名后缀路由
